...
void furthest_point_sampling_weights_kernel_wrapper(int b, int n, int m,
                                            const float *xyz, const float *weights, float *temp,
                                            int *idxs);

void furthest_point_sampling_hybrid_kernel_wrapper(int b, int n, int m,
                                            const float *xyz, const float *xyz_offset, float *temp,
                                            int *idxs, float ratio);
...
at::Tensor furthest_point_sampling_weights(at::Tensor points, at::Tensor weights, const int nsamples) {
  ...
  if (points.is_cuda()) {
    furthest_point_sampling_weights_kernel_wrapper(
        points.size(0), points.size(1), nsamples, points.data<float>(), weights.data<float>(),
        tmp.data<float>(), output.data<int>());
  } else {
    AT_ASSERT(false, "CPU not supported");
  }
...
at::Tensor furthest_point_sampling_hybrid(at::Tensor points, at::Tensor points_offset, const int nsamples, const float ratio) {
  ...
  CHECK_CONTIGUOUS(points_offset);
  CHECK_IS_FLOAT(points_offset);
  ...
  if (points.is_cuda()) {
    furthest_point_sampling_hybrid_kernel_wrapper(
        points.size(0), points.size(1), nsamples, points.data<float>(), points_offset.data<float>(),
        tmp.data<float>(), output.data<int>(), ratio);
  } else {
    AT_ASSERT(false, "CPU not supported");
  }
...
at::Tensor furthest_point_sampling_with_dist(at::Tensor points, const int nsamples) {
  ...
  if (points.is_cuda()) {
    furthest_point_sampling_with_dist_kernel_wrapper(
        points.size(0), points.size(1), nsamples, points.data<float>(),
        tmp.data<float>(), output.data<int>());
  } else {
    AT_ASSERT(false, "CPU not supported");
